...
1  . what is the total amount each customer spent on zomat
...
from sales a inner join
 product b
  ...
  /////////////////////////////////////////////////////////////
   ...
   (select c.*,rank() 
   over(partition by userid 
   order by created_date desc)rnk from 
...
/////////////////////////////////////////////////////////////
   ...
   /*3*/Select userid,count(created_date)order_purchased,sum(price) total_amt_spent 
   from 
   /*2*/(select c.*,d.price
   from
   /*1*/(Select a.userid,a.created_date,a.product_id,b.gold_signup_date 
   from sales a 
   inner join
   goldusers_signup b 
   on a.userid = b.userid   
   ...
   inner join 
  /*4*/ product d on c.product_id = d.product_id)e
   /*5*/group by userid
/////////////////////////////////////////////////////////////
   9 .if buying each product genrates points for eg 5rs=2 Zomato point and each product has differnt purchasing points
   for eg for 5rs=1 Zomato point ,for p2 10rs= 5zomato point andd p3 5rs=1 zomato point  2rs=1zomato point 

   .calculate points collected by each customers and for which product most points have been given till now.

   select * from
   (select * , rank() over
   (order by total_point_earned desc) rnk
   from
  ( select product_id,sum(total_points) total_point_earned from
   ...
   10. in the first one year after a customer joins the gold program (including their join date) irrespective
    of what the cutomer has purchased they earn 5 zomato points for every 10 rs spent who earned more 10 or 3
	and what was their points earnings in their first yr?
	
   /*pour 0.5 zp=1rs*/

   select c.*,d.price*0.5 total_p_earned from 
   (
   Select a.userid,a.created_date,a.product_id,b.gold_signup_date 
   from sales a 
   inner join
...
/////////////////////////////////////////////////////////////
   ...
   order by created_date) rnk from sales 
/////////////////////////////////////////////////////////////

   12 . rank all the transaction for each member whenever they are a zomato for every non gold member transction mark as na
